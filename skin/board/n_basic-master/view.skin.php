...
add_stylesheet('<link rel="stylesheet" href="'.$board_skin_url.'/style.css?ver=180610">', 0);
?>
...
<div class="board-viewer">
	<hr class="padding">
	<div class="subject">
		<? if ($is_category) { ?><em><?=$view['ca_name']?></em><? } ?>
		<strong><?=get_text($view['wr_subject'])?></strong>
	</div>

	<div class="info">
		<span><? echo $view['name'] ?><? if ($is_ip_view) { echo "&nbsp;($ip)"; } ?></span>
		<span><? echo date("ymd H:i", strtotime($view['wr_datetime'])) ?></span>
	</div>
	...
	<div class="contents">
		...
		<div id="bo_v_con"><?php
			// 콜링 설정
			$view['content'] = str_replace("[[", "<span class='member_call txt-point'>", $view['content']);
			$view['content'] = str_replace("]]", "</span>", $view['content']);
