...
if(!isset($write['wr_content_story'])) { 
	sql_query(" ALTER TABLE `{$write_table}` ADD `wr_content_story` text NOT NULL AFTER `wr_content` ");
}
if(!isset($write['wr_log_type'])) { 
	sql_query(" ALTER TABLE `{$write_table}` ADD `wr_log_type` varcharacter(255) NOT NULL default '' AFTER `wr_type` ");
}
...
if(!$is_error) { 
	...
	if($character['ch_id']) { 
		// 사용가능 아이템 검색
		$temp_sql = "select it.it_id, it.it_name, inven.in_id from {$g5['inventory_table']} inven, {$g5['item_table']} it where it.it_id = inven.it_id and it.it_use_mmb_able = '1' and inven.ch_id = '{$character['ch_id']}' order by it.it_name asc";
	...
	if(!$write['wr_log_type']) {
		$write['wr_log_type'] = get_cookie("write_log_type") ? get_cookie("write_log_type") : "img";
	}
	...
	<div id="load_log_board">
		<section id="bo_w" class="mmb-board">
			...
			<div class="write-title">로그작성</div>
			<select name="wr_log_type" id="wr_log_type" class="full">
				<option value="img" <?=$write['wr_log_type'] == 'img' ? "selected" : ""?>>이미지 로그</option>
				<option value="story" <?=$write['wr_log_type'] == 'story' ? "selected" : ""?>>글 로그</option>
			</select>

			<div id="log_img_box" class="log_box" style="display: <?=$write['wr_log_type'] == 'img' ? "block" : "none"?>;">
				<!-- LOG 등록 부분 -->
			...
			<div id="log_story_box" class="log_box"  style="display: <?=$write['wr_log_type'] == 'story' ? "block" : "none"?>;">
				<?php if($write_min || $write_max) { ?>
				<!-- 최소/최대 글자 수 사용 시 -->
				<p class="txt-center help">
					최소 <strong><?php echo $write_min; ?></strong>글자 이상
					<? if($write_max && $write_min) { ?>
						,
					<? }
					if($write_max) { ?>
						최대 <strong><?php echo $write_max; ?></strong>글자 이하까지
					<? } ?>
					글을 쓰실 수 있습니다.
				</p>
				<?php } ?>
				<textarea id="wr_content_story" name="wr_content_story" placeholder="로그를 작성해 주세요"><?=$write['wr_content_story']?></textarea>
				<?php if($write_min || $write_max) { ?>
				<!-- 최소/최대 글자 수 사용 시 -->
			...
			<div id="log_func_box" class="theme-box">
			<?php if ($is_category) { ?>
			...
			<div class="comments">
				<div class="write-title">오너 코멘트</div>
				<textarea id="wr_content" name="wr_content"><?=$write['wr_content']?></textarea>
				<p class="txt-center help">해시태그 : #해시태그내용 / 로그링크 : @로그번호 / 멤버알람 : [[닉네임]]</p>
			</div>
...
<script>
	...
	check_byte("wr_content_story", "char_count");

	$(function() {
		$("#wr_content_story").on("keyup", function() {
			check_byte("wr_content_story", "char_count");
		});
	});
	<?php } ?>
	...
	function fwrite_submit(f)
	{
		...
		if(f.wr_log_type.value == 'story') {
			if (document.getElementById("char_count")) {
				if (char_min > 0 || char_max > 0) {
					var cnt = parseInt(check_byte("wr_content_story", "char_count"));
...
$('#log_func_box select').change(function() {
	$('#log_func_box select').find("option").attr('disabled', false);
	$('#log_func_box select').each(function() {
		if($(this).val()) { 
			$('#log_func_box select').not(this).find("option[value="+ $(this).val() + "]").attr('disabled', true);
		}
	});
});

$('#wr_log_type').change(function() {
	var log_type = $(this).val();
	$('.log_box').hide();
	$('#log_'+log_type+'_box').show();
});
