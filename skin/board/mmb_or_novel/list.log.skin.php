...
if($list_item['wr_log_type'] == 'img') { 
	...
	/* 글로그 셋팅 부분 */
	$image_url = conv_content($list_item['wr_content_story'], 0, 'wr_content_story');
	$image_width = 500;
	$image_height = 0;
}
...
<div class="item <?=$h_class?> <?=$list_item['wr_log_type']?>" id="log_<?=$list_item['wr_id']?>">
		...
		<div class="ui-pic <?=$no_member_class?>" data-width="<?=$data_width?>">
			...
			<? } else { ?>
				...
				<? } else { ?>
						...
						<?	if($log_class) { ?>
							<a href="#" class="ui-open-log slide-pop-open-btn">OPEN</a>
						<? } ?>
						<?	if($log_class) { ?>
							<a href="#" class="ui-open-log slide-open-btn">OPEN</a>
						<? } ?>
						<?	if($blind_class) { ?><a href="#" class="ui-remove-blind"><span>해당 로그는 필터 된 로그 입니다.<br />확인을 원하실 경우 클릭해주세요.</span></a><? } ?>
				<? } ?>
			<? } ?>
			</div>
			<!-- // 로그 상단 영역 -->
