...
<? } ?>
	...
	<h4>
		<form name=fsearch method=get style="margin:0px;">
			...
			<button type="submit" class="ui-btn">검색하기</button>
		</form>
	</h4>
	</div>
		...
		<li> 
			...
			<form name="fboardlist" method="post" action="<?=$board_skin_url?>/password.php" style="margin:0">
				...
				<div class="qna-content">
					...
					<fieldset class="ui-qna-list-password">
						...
						<button type="submit" class="ui-btn">ENTER</button>
					</fieldset>
					<? } else { ?>
			...
			<? 
				if(strstr($lists[$ii]['wr_option'], 'secret') && !$is_admin && !$is_open) { 
					...
					<p class="announce">*답변이 완료되었습니다.</p><?}
				} else { 
	...
	<?	} 
		...
		if (!$member['mb_id']) // 비회원일 경우에만
			echo "<script language='javascript' src='".G5_URL."/js/md5.js'></script>\n";
	?>
