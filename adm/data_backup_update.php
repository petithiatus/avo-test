...
$backup_path = G5_ADMIN_PATH."/backup";
$backup_url = G5_ADMIN_URL."/backup";

@mkdir($backup_path, G5_DIR_PERMISSION);
@chmod($backup_path, G5_DIR_PERMISSION);


function backup_tables($host,$user,$pass,$name,$tables = '*', $is_viewer = false) {
	$link = sql_connect($host,$user,$pass,$name);
	sql_select_db($name,$link);
